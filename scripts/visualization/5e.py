...
if df['Trigger_Temperature'].dtype == 'object':
    df[('Trigger_'
        'emperature')] = df['Trigger_Temperature'].astype(str).str.rstrip('%').astype(float)

# Mapping of Co content for each material
co_map = {
    'LFP': 0,
    'NCM-111': 0.33, 'NCM111': 0.33,
    'NCM-523': 0.20, 'NCM523': 0.20,
    'NCM-622': 0.20, 'NCM622': 0.20,
    'NCM-811': 0.10, 'NCM811': 0.10,
    'NCA': 0.10
}

df['Co_Content'] = df['Material'].astype(str).str.strip().map(co_map)
df_plot = df.dropna(subset=['Co_Content', 'Trigger_Temperature']).copy()

# =========================================================
# Step B: Calculate rank and jitter
# =========================================================
unique_co_values = sorted(df_plot['Co_Content'].unique())
rank_map = {val: i for i, val in enumerate(unique_co_values)}
df_plot['X_Rank'] = df_plot['Co_Content'].map(rank_map)

sorted_materials_for_legend = (
    df_plot.sort_values('Co_Content')['Material'].unique().tolist()
)
...
# =========================================================
ax.set_xticks(range(len(unique_co_values)))
ax.set_xticklabels(unique_co_values, fontweight='bold')
ax.tick_params(
    axis='both',
    which='major',
    labelsize=24,
...
ax.set_xlabel("Co Content (%)", fontsize=36, fontweight='bold', labelpad=20)
...
ax.set_ylim(0, 450)
ax.set_xlim(-0.6, len(unique_co_values) - 0.4)
...
plt.tight_layout()
plt.savefig('supp_fig_s2_co_content_vs_trigger_temperature.png', dpi=300, bbox_inches='tight')
plt.show()
